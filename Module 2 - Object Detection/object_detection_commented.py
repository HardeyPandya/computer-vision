...
from torch.autograd import Variable
    #autograd is for gradient descent
    #Variable class contains tensor and gradient
import cv2 #to draw rectangles
from data import BaseTransform, VOC_CLASSES as labelmap
    #from folder data;
    #BaseTransform is analog to BLOB in yolo
    #classes are maped
from ssd import build_ssd
    #Constructor of ssd neural network
import imageio
    #Video processing
...
def detect(frame, net, transform): 
    ...
    frame_t = transform(frame)[0] # We apply the transformation to our frame.
        #Similar to BLOB from YOLO
    x = torch.from_numpy(frame_t).permute(2, 0, 1)
        #Convert from numpy ndarray (RGB image) to pytorch tensor
        #permute tensor from RGB to BRG which is the input to ssd
    x = Variable(x.unsqueeze(0)) 
        #We add a fake dimension corresponding to the batch
        #0 is the index of the extra dimensionM
    y = net(x) 
        # We feed the neural network ssd with the image and we get the output y.
    detections = y.data 
        # detections = [batch, num_classes, num_occurence, (score, x0, y0, x1, x1)]
        # We create the detections tensor contained in the output y.
    scale = torch.Tensor([width, height, width, height]) 
        #Used for normalization of box dimensions
...
net.load_state_dict(torch.load('data/ssd300_mAP_77.43_v2.pth', map_location = lambda storage, loc: storage)) # We get the weights of the neural network from another one that is pretrained (ssd300_mAP_77.43_v2.pth).
...
reader = imageio.get_reader('data/funny_dog.mp4') # We open the video.
fps = reader.get_meta_data()['fps'] # We get the fps frequence (frames per second).
writer = imageio.get_writer('data/output.mp4', fps = fps) # We create an output video with this same fps frequence.
